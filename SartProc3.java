...
public class SartProc3 {
    ...
    public static void main(String[] args) {
        ...
        for (Map.Entry<Integer, String> bpEntry : baseproduct.entrySet()) {
            ...
            if (bpEntryValue.contains("\uFFFD")){
                bpEntryValue = bpEntryValue.replace("\uFFFD", "\u00A3");
            }
            ...
            for (Map.Entry<String, String> stEntry : sartMultiwordSorted.entrySet()) {
                ...
                if (
                    (bpEntryValue.equals(stEntryKey) || bpEntryValue.contains(stEntryKey + " ") || bpEntryValue.contains(" " + stEntryKey)) && 
                    (bpEntryValue.startsWith(stEntryKey) || bpEntryValue.endsWith(stEntryKey)) &&
                    bpEntryValue.contains(stEntryKey)
                    ) 
                {
                    //Replace the matched SART in BaseProduct input with the SART value.
                    //Get the start and indexes of this substring
                    if (!baseProductIndexes.containsKey(bpEntryKey)){
                        baseProductMultiWordUpdated.put(bpEntryKey, stEntryValue);
                        baseProductIndexes.put(bpEntryKey, findSubstringIndices(bpEntryValue, stEntryKey));
                    }
                }
                else if (
                    (bpEntryValue.equals(stEntryKey) || bpEntryValue.contains(" " + stEntryKey + " ")) && 
                    bpEntryValue.contains(stEntryKey)
                )
                {
                     if (!baseProductIndexes.containsKey(bpEntryKey)){
    ...
    public static String toTitleCase(String givenString) {
        ...
        for (String str : arr) {
            if (str.length() != 0) {
                ...
                } else if (str.matches(".*(\\d+H/\\d+|\\d+(G|C|L|M|W|H|D|S))$") || str.contains("&") || str.contains("H+M")) {
                    sb.append(str).append(" ");
                } else if (str.matches(".+-.+")) {
